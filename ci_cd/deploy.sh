...
docker_login_tag_push() {
  ...
  docker tag selenium:latest furkandogan/selenium:${TRAVIS_TAG}
  docker tag selenium:latest furkandogan/selenium:latest

  # e.g. TAG_VERSION_MAJOR="3"
  TAG_VERSION_MAJOR="${TRAVIS_TAG::1}"
  echo "TAG_VERSION_MAJOR=${TAG_VERSION_MAJOR}"
  docker tag selenium:latest furkandogan/selenium:${TAG_VERSION_MAJOR}
  # e.g. TAG_VERSION_MAJ_MINOR="3.3"
  TAG_VERSION_MAJ_MINOR="${TRAVIS_TAG::3}"
  echo "TAG_VERSION_MAJ_MINOR=${TAG_VERSION_MAJ_MINOR}"
  docker tag selenium:latest furkandogan/selenium:${TAG_VERSION_MAJ_MINOR}
  ...
  docker tag selenium:latest furkandogan/selenium:${TAG_VERSION_MAJ_MIN_PATCH}

  # Push this version with all the corresponding tags
  docker push furkandogan/selenium:${TAG_VERSION_MAJOR}
  docker push furkandogan/selenium:${TAG_VERSION_MAJ_MINOR}
  docker push furkandogan/selenium:${TAG_VERSION_MAJ_MIN_PATCH}
  docker push furkandogan/selenium:${TRAVIS_TAG} | tee docker_push.log
  docker push furkandogan/selenium:latest

  # Update Docker README.md badges
  curl -XPOST "https://hooks.microbadger.com/images/furkandogan/selenium/2blr1ujGSTcUtm3HnU0-h5m6yLY=" || true
}
...
git_co_fetch_merge_stash() {
  ...
  git remote add github "https://elgalubot:${GITHUB_TOKEN}@github.com/furkandogan/docker-selenium.git"
  git fetch github
  # git stash save || true
